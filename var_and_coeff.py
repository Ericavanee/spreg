...
def get_asymp_var(estimated_params, x,y, n, T, k, W_ls):
    r"""get asymptotic variance
     
    The main idea of this function is to calculate the asympototic variance given estimated QMLE parameters. 
    
    Parameters
    ----------
    estimated_params : 1D numpy array; float64
        Estimated parameters in order of sigma, lam, gamma, rho, and beta. Beta is included in its raveled form. For example, if sigma, lam, gamma, rho = 0.5 and beta = [1,1], then estimated_params is written as np.array(0.5,0.5,0.5,0.5,1,1).
    x : list; numpy array of shape n*k; float64
        List of length T of Variable X with k-feature in the QMLE model. The list starts with X1.
    y: list; numpy array of shape n*1; float64
        List of length T+1 response variable Y in the QMLE model. The list starts with Y0.
    n : int
        Number of items. 
    T : int
        Number of time points.
    k : int
        Number of features of the x-variable.
    W_ls : list; numpy array of shape n*n; float64
        List of length T+1 of exogenously defined weight matrices. The list starts with W0.     
        
    Returns
    -------
    asymp_var: numpy array of shape (k+4)*(k+4); float64
        Asymptotic variance of estimated parameters.
     """
...
def get_var_score(x,y, W_ls,lam,sigma,gamma,rho,beta,k,n,T):
    r"""get variance score matrix
     
    The main idea of this function is to calculate the variance score matrix given estimated QMLE parameters. 
    ...
    beta : 1Darray; float64
        1Darray of length k; represents the regression function parameter in equation (1)
    k : int
        Number of features of the x-variable. 
    n : int
        Number of items. 
    ...
    Returns
    -------
    numpy array of shape (k+4)*(k+4); float64
        Returns the variance score matrix
     """
    info_mat = get_info_mat(x,y, W_ls,lam,sigma,gamma,rho,beta,k,n,T)
...
def get_omega(x,y, W_ls,lam,sigma,gamma,rho,beta,k,n,T):
    ...
    The main idea of this function is to calculate the omega matrix given estimated QMLE parameters. 
...
def get_omega_first_part(x,y,c0, V_ls, W_ls,lam,sigma,gamma,rho,beta,k,n,T,mu3):
    ...
    The main idea of this function is to calculate the first component of the omega function. 
